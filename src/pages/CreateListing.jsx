...
const CreateListing = () => {
    ...
    const onSubmit = async (e) => {
        ...
        if (discountedPrice >= regularPrice) {
            setLoading(false)
            toast.error('Snižena cena mora biti niža od redovne cene.')
            return
        }
        //set max 6 images
        if (images.length > 6) {
            setLoading(false)
            toast.error('Max 6 slika')
            return
        }
                ...
                uploadTask.on('state_changed',
                    (snapshot) => {
                        ...
                        switch (snapshot.state) {
                            case 'paused':
                                console.log('Otpremanje pauzirano');
                                break;
                            case 'running':
                                console.log('Otpremanje u toku');
                                break;
                        }
                    },
                    (error) => {
        ...
        const imgUrls = await Promise.all(
            ...
            setLoading(false)
            toast.error('Slike nisu otpremljene')
            return
        })
    ...
    return (
        <div className='profile'>
            <header>
                <p className='pageHeader'>Kreiraj listu</p>
            </header>
            ...
            <main>
                <form onSubmit={onSubmit}>
                    ...
                    <div className='formRooms flex'>
                        ...
                        <div>
                            <label className='formLabel'>Kupatila</label>
                            <input
                                className='formInputSmall'
                                type='number'
                                id='bathrooms'
                    ...
                    <label className='formLabel'>Parking mesto</label>
                    <div className='formButtons'>
                        <button
